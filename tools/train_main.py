        ...
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in progress_bar:
                ...
                progress_bar.set_description(
                    f"epoch: {epoch} iter: {iteration}/{max_iter} "+''.join([f'{k}: {v.item():.4f}, ' for k,v in loss_dict.items()])
                )
                periodic_checkpointer.step(iteration)

            new_teacher_dict = update_teacher_model(model_student, model_teacher, keep_rate=0.75)

...
class Res5ROIHeads(ROIHeads):
    ...
    def forward(
        ...
        del targets
        proposal_boxes = [x.proposal_boxes for x in proposals] #300 x 4
        
        box_features = self._shared_roi_transform(                          ##### ROI Pool - 300x2048x7x7, self.in_features - 'res4'
            [features[f] for f in self.in_features], proposal_boxes
        )
        # C+1 categories, bbox for each category except background
        ...
        else: #for teacher on training, validation / for student on validation

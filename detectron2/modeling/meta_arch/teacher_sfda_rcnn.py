...
__all__ = ["teacher_sfda_RCNN"]


@META_ARCH_REGISTRY.register()
class teacher_sfda_RCNN(nn.Module):
    """
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], mode="test", iteration=0):
        """
        Args:
        ...
        if self.proposal_generator is not None:
            ...
            proposals = [x["proposals"].to(self.device) for x in batched_inputs] #300
            proposal_losses = {}
        
        #Res5ROIHeads
        results, detector_losses = self.roi_heads(images, features, proposals, gt_instances) #results here is final 
